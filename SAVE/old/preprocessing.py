...
def plot_cumulative_explained_variance(data):
    '''
    Plot cumulative variance to explain how the number of features is chosen

    Parameters:
        model_data (dataframe): data which PCA model is fitted
    '''
...
def create_pca_model(model_data):
    '''
    Create a PCA model that preserves 95% of variance
...
def pca_dimensionality_reduction(data):
    ...
    Parameters:
        data (dataframe): data to reduct
    
    Returns:
        df_pca (dataframe): reducted data
    '''
