...
def _find_best_model(X_train, y_train, X_validate, y_true):
    ...
    Returns:
        best_model (RandomForestClassifier): the model with the higher True Positive Rate
    '''
    tuned_n_estimators = [25, 50, 100]
    tuned_criterion = ['gini', 'entropy']
    tuned_max_depth = [2, 5, 10, 20]
    tuned_max_features = ['sqrt', 'log2', None, 0.1, 1, 10]

    total_model = len(tuned_n_estimators) * len(tuned_criterion) * len(tuned_max_depth) * len(tuned_max_features)
    i_model = 1
    
    best_tnr = 0
    best_tpr = 0
    best_model = None
    for n_esitmators_value in tuned_n_estimators:
        for criterion_value in tuned_criterion:
            for max_depth_value in tuned_max_depth:
                for max_features_value in tuned_max_features:
                    print('Trying model {0}/{1} ...'.format(i_model, total_model))
                    i_model += 1
                    # train the model
                    model = RandomForestClassifier(
                        n_estimators=n_esitmators_value, 
...
def get_result(X_train, y_train, X_validate, y_validate, X_test, y_true):
    ...
    # save the model
    dump(model, '../../models/binary_classifiers/random_forest.bin', compress=True)
...
#     '''
#     Perform cross validation to turn model parameters

#     Parameters:
#         X (ndarray of shape (n_samples, n_features)): independent columns
#         y (ndarray of shape (n_samples,)): target values

#     Returns:
#         best_model (RandomForestClassifier): model with best parameters
#         best_scaler (MinMaxScaler): scaler
#     '''
#     # X and y are <class 'numpy.ndarray'>
#     tuned_n_estimators = [25, 50, 75, 100, 125, 150]
#     num_splits = len(tuned_n_estimators)
#     s_k_fold = StratifiedKFold(n_splits=num_splits, shuffle=True, random_state=10)
#     best_model = None
#     best_scaler = None
#     best_score = 0
#     index = 0
#     for train_index, validate_index in s_k_fold.split(X, y):
#         # pre-processing
...
#         X_validate = scaler.transform(X_validate)
#         X_train = scaler.transform(X_train)
#         smote = SMOTE(sampling_strategy=1/9, random_state=0)
#         X_res, y_res = smote.fit_resample(X_train, y_train)
#         del X_train, y_train
#         # train the model
#         model = RandomForestClassifier(n_estimators=tuned_n_estimators[index], criterion='gini', random_state=0, verbose=1)
#         model.fit(X_res, y_res)
#         # predict
#         y_pred = model.predict(X_validate)
#         y_prob = model.predict_proba(X_validate)
#         _, _, _, tpr = confusion_matrix(y_true=y_validate, y_pred=y_pred, normalize='true').ravel()
#         roc_auc = roc_auc_score(y_validate, y_prob[:, 1])
#         score = tpr
#         if score > best_score:
#             best_model = model
#             best_scaler = scaler
#             best_score = score
#         print('TPR: {0}\tROC AUC: {1}'.format(tpr, roc_auc))
#         index += 1
#     return best_model, best_scaler

# def get_result(training_set, test_set):
#     '''
#     Predict label of test set and save the result
...
#         test_set (DataFrame): raw test set
#     '''
#     # separe dependent column from independent columns 
#     X_train = training_set.iloc[:, 0:-1].values
#     y_train = training_set.iloc[:, -1].values
#     X_test = test_set.iloc[:, 0:-1].values
#     y_test = test_set.iloc[:, -1].values
#     # perform cross validation
#     model, scaler = cross_validation(X_train, y_train)
#     # scale test set
#     X_test = scaler.transform(X_test)
...
#     # print result
#     conf_matrix = confusion_matrix(y_true=y_test, y_pred=y_pred, normalize=None).ravel()
#     rates_matrix = confusion_matrix(y_true=y_test, y_pred=y_pred, normalize='true').ravel()
#     logger.info('RANDOM FOREST')
#     logger.info('TN: {0}, FP: {1}, FN: {2}, TP: {3}'.format(conf_matrix[0], conf_matrix[1], conf_matrix[2], conf_matrix[3]))
#     logger.info('TNR: {0:.5f}, TPR: {1:.5f}\n'.format(rates_matrix[0], rates_matrix[3]))
#     # save the model
...
#     test_set = pd.read_pickle('../../pickle/raw_testset.pkl')
#     # result
#     get_result(training_set, test_set)
